...
static inline struct netmidi2_session *netmidi2_match_session(
...
static inline struct netmidi2_session *netmidi2_try_alloc_session(
	struct netmidi2_ep *ep,
	struct sockaddr *peer_addr,
	socklen_t peer_addr_len
)
...
static inline struct netmidi2_session *netmidi2_alloc_session(
	struct netmidi2_ep *ep,
	struct sockaddr *peer_addr,
	socklen_t peer_addr_len
)
{
	struct netmidi2_session *res = netmidi2_try_alloc_session(ep, peer_addr, peer_addr_len);
	if (! res) {
		netmidi2_free_inactive_sessions(ep);
		res = netmidi2_try_alloc_session(ep, peer_addr, peer_addr_len);
	}
...
static void netmidi2_service_handler(struct net_socket_service_event *pev)
{
	...
	while (
		rxbuf->len >= 4 &&
		netmidi2_dispatch_command_packet(ep, &peer_addr, peer_addr_len, rxbuf) == 0
	);
