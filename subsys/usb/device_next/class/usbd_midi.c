...
{
	const struct device *dev = usbd_class_get_private(class_data);
	enum usbd_midi_status status = USBD_MIDI_DISABLED;
	struct usbd_midi_data *data = dev->data;
	...
	case ALT_USB_MIDI_2:
		...
		status = USBD_MIDI_2_ENABLED;
		LOG_INF("%s set USB-MIDI2.0 altsetting", dev->name);
		break;
	}

	if (data->ops.status_change_cb) {
		data->ops.status_change_cb(dev, status);
	}
}
...
static void usbd_midi_class_enable(struct usbd_class_data *const class_data)
{
	...
	if (data->altsetting == ALT_USB_MIDI_2 && data->ops.status_change_cb) {
...
static void usbd_midi_class_disable(struct usbd_class_data *const class_data)
{
	...
	if (data->ops.status_change_cb) {
		data->ops.status_change_cb(dev, USBD_MIDI_DISABLED);
	}
			       ...
			       struct net_buf *const buf)
{
	...
	if (data->altsetting != ALT_USB_MIDI_2 ||
	    ...
	    setup->wValue != ((CS_GR_TRM_BLOCK << 8) | ALT_USB_MIDI_2)) {
		errno = -ENOTSUP;
		return 0;
	}
