				...
				audio_channel_t channel, audio_property_value_t val)
{
	...
	if (property == AUDIO_PROPERTY_OUTPUT_MUTE) {
		...
		case AUDIO_CHANNEL_ALL:
			dac_channel_mute = HEADPHONES_A_MUTE | HEADPHONES_B_MUTE | SPEAKER_A_MUTE |
					   SPEAKER_B_MUTE;
			break;
		...
		volume_scaled = 192 * val.vol / 100;
		return cs43l22_write(&cfg->i2c, reg, 192 - volume_scaled);
	}
...
#define CS43L22_INIT(inst)                                                                         \
	static const struct cs43l22_config cs43l22_config_##inst = {                               \
		...
		.reset_gpio = GPIO_DT_SPEC_INST_GET(inst, reset_gpios),                            \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, cs43l22_init, NULL, NULL, &cs43l22_config_##inst, POST_KERNEL, \
			      CONFIG_AUDIO_CODEC_INIT_PRIORITY, &cs43l22_api);
