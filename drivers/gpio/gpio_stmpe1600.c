...
static int write_reg16(const struct device *dev, uint8_t reg, uint16_t value)
{
	...
	LOG_DBG("STMPE1600[0x%02X]: write REG[0x%02X..0x%02X] = %04x", i2c_addr, reg, reg+1, value);
	...
	if (ret != 0){
		LOG_ERR("STMPE1600[0x%02X]: write error REG[0x%02X..0x%02X]: %d", i2c_addr, reg, reg+1, ret);
	}
	return ret;
}
...
static int read_reg16(const struct device *dev, uint8_t reg, uint16_t *value)
{
	...
	if (ret != 0){
		LOG_ERR("STMPE1600[0x%02X]: read error REG[0x%02X..0x%02X]: %d", i2c_addr, reg, reg+1, ret);
	} else {
		*value = sys_le16_to_cpu(transfer_data);
		LOG_DBG("STMPE1600[0x%02X]: read REG[0x%02X..0x%02X] => %04x", i2c_addr, reg, reg+1, *value);
	}
	return ret;
}
