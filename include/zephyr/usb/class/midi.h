 ...
 * @param[in]  ump_ptr    Pointer to Universal MIDI Packet
 */
#define UMP_MT(ump_ptr) \
	((ump_ptr)->data[0] >> 28)
 ...
 * @see ump112: 2.1.4 Message Type (MT) Allocation
 */
#define UMP_NUM_WORDS(ump_ptr) \
	(1 + ((UMP_NUM_WORDS_LOOKUP_TABLE >> (2 * UMP_MT(ump_ptr))) & 3))

/**
 * @brief      MIDI group field of a Universal MIDI Packet
 * @param[in]  ump_ptr    Pointer to Universal MIDI Packet
 */
#define UMP_GROUP(ump_ptr) \
	(((ump_ptr)->data[0] >> 24) & 0x0f)

/**
 * @brief      Status byte of a MIDI channel voice or system message
 * @param[in]  ump_ptr    Pointer to Universal MIDI Packet (containing a MIDI1 event)
 */
#define UMP_MIDI_STATUS(ump_ptr) \
	(((ump_ptr)->data[0] >> 16) & 0xff)
/**
 * @brief      Command of a MIDI channel voice message
 * @param[in]  ump_ptr    Pointer to Universal MIDI Packet (containing a MIDI event)
 * @see usb_midi_cmd
 */
#define UMP_MIDI_COMMAND(ump_ptr) \
	(UMP_MIDI_STATUS(ump_ptr) >> 4)
/**
 * @brief      Channel of a MIDI channel voice message
 * @param[in]  ump_ptr    Pointer to Universal MIDI Packet (containing a MIDI event)
 */
#define UMP_MIDI_CHANNEL(ump_ptr) \
	(UMP_MIDI_STATUS(ump_ptr) & 0x0f)
/**
 * @brief      First parameter of a MIDI1 channel voice or system message
 * @param[in]  ump_ptr     Pointer to Universal MIDI Packet (containing a MIDI1 message)
 */
#define UMP_MIDI1_P1(ump_ptr) \
	(((ump_ptr)->data[0] >> 8) & 0x7f)
/**
 * @brief      Second parameter of a MIDI1 channel voice or system message
 * @param[in]  ump_ptr     Pointer to Universal MIDI Packet (containing a MIDI1 message)
 */
#define UMP_MIDI1_P2(ump_ptr) \
	((ump_ptr)->data[0] & 0x7f)
