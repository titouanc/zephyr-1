...
struct ump_midi1 {
	...
	uint8_t p2;	     //!< The 2nd MIDI1 parameter
	uint8_t p1;          //!< The 1st MIDI1 parameter
	unsigned channel: 4; //!< The MIDI1 channel number
	unsigned status: 4;  //!< The MIDI1 status nibble (command)
	unsigned group: 4;   //!< The UMP group to which this message belongs
	unsigned mt: 4;      //!< The UMP message type (MT_MIDI1_CHANNEL_VOICE)
#else
	unsigned mt: 4;      //!< The UMP message type (MT_MIDI1_CHANNEL_VOICE)
	unsigned group: 4;   //!< The UMP group to which this message belongs
	unsigned status: 4;  //!< The MIDI1 status nibble (command)
...
union ump {
	...
	struct {
		...
		unsigned group: 4; //!< The UMP group to which this message belongs
		unsigned mt: 4;    //!< The UMP message type
#else
		unsigned mt: 4;    //!< The UMP message type
		unsigned group: 4; //!< The UMP group to which this message belongs
#endif
